...
void save_motion_thread_auto(std::deque<int> &motion, cv::VideoCapture &movie, std::string path, double fps, int start, int thread_no){
	...
	std::wstring stemp = std::wstring(directory_path.begin(), directory_path.end());
	LPCWSTR sw = stemp.c_str();
	CreateDirectory(sw,NULL); // tworzenie katalogu odpowiadającemu nazwie przetwarzanego pliku wideo
...
int main(int argc, char *argv[])
{
	...
	parametry["threads"] = 4;
	std::string path = "Casptured by Motion Detection 2.avi";
